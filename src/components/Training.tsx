...
export type TrainingTypeValue = (typeof TrainingType)[keyof typeof TrainingType]
...
const uploadTraining = (training: {
	...
	const trainings: Training[] = localStorage.getItem('trainings')
	...
	const newTraining: Training = {
		...
		userId: 'local-user',
		type: training.type,
		referenceMilliseconds: training.referenceMilliseconds,
		createdAt: new Date(),
	}
...
const Training: FC<TrainingProps> = ({
	...
	return (
		<>
			<Spacer height={'h-6'} />
			<div
				className={`box-content bg-cover bg-center ${
					isFull
					...
					trainingType === TrainingType.O2
						? `bg-[url('/assets/o2-training.jpg')]`
						: `bg-[url('/assets/co2-training.jpg')]`
				} ${transition}`}
			...
			>
				...
				<div
					className={`h-fit flex-1 bg-stone-600 opacity-80 text-white ${
						isFull
				...
				>
					...
					>
						<div className="mb-5">
							<h3
								className={`${isFull ? 'text-4xl' : 'text-xl'}`}
							>
								{title}
							</h3>
							{hasRecord && (
